...
def _build_augmented_input(
    sdata: sd.SpatialData, shapes_gdf: gpd.GeoDataFrame, out_path: Path
) -> Path:
    """Write a SpatialData zarr that segger can consume: original transcripts +
    morphology + the freshly computed nucleus polygons."""
    shapes_parsed = ShapesModel.parse(shapes_gdf, transformations={"global": Identity()})
    augmented = sd.SpatialData(
        images={"morphology_mip": sdata.images["morphology_mip"]},
        points={"transcripts": sdata.points["transcripts"]},
        shapes={"nucleus_boundaries": shapes_parsed},
        tables={"table": sdata.tables["table"]},
    )
    if out_path.exists():
        shutil.rmtree(out_path)
    augmented.write(str(out_path), overwrite=True)
    return out_path


def _run_segger(input_zarr: Path, output_dir: Path) -> Path:
    cmd = [
        "segger", "segment",
        "-i", str(input_zarr),
        "-o", str(output_dir),
        ...
        "--spatialdata-nucleus-shapes-key", "nucleus_boundaries",
    ]
...
# --- Step 2: write augmented SpatialData and run segger ---
_build_augmented_input(sdata, shapes_gdf, augmented_zarr)
seg_pq = _run_segger(augmented_zarr, segger_out_dir)
